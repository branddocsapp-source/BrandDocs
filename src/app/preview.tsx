...
const visitingCardStatusOptions: {
  value: VisitingCardStatus;
  label: string;
  description: string;
}[] = [
  { value: "draft", label: "Draft", description: "Still editable" },
...
export default function PreviewScreen() {
  ...
  const [visitingCard, setVisitingCard] = useState<VisitingCardRecord | null>(
    null,
  );
  const [receipt, setReceipt] = useState<ReceiptRecord | null>(null);
  const [selectedStatus, setSelectedStatus] = useState<InvoiceStatus>("draft");
  const [selectedQuotationStatus, setSelectedQuotationStatus] =
    useState<QuotationStatus>("draft");
  const [selectedLetterheadStatus, setSelectedLetterheadStatus] =
    useState<LetterheadStatus>("draft");
  const [selectedVisitingCardStatus, setSelectedVisitingCardStatus] =
    useState<VisitingCardStatus>("draft");
  ...
  useEffect(() => {
    ...
    async function hydrateInvoicePreview() {
      ...
      const savedInvoice = await loadInvoiceById(
        auth.currentUser,
        profile,
        invoiceId,
      );
    ...
    async function hydrateQuotationPreview() {
      ...
      const profile = await loadBusinessProfile(auth.currentUser);
      const savedQuotation = await loadQuotationById(
        auth.currentUser,
        profile,
        quotationId,
      );
    ...
    async function hydrateLetterheadPreview() {
      ...
      const profile = await loadBusinessProfile(auth.currentUser);
      const savedLetterhead = await loadLetterheadById(
        auth.currentUser,
        profile,
        letterheadId,
      );
    ...
    async function hydrateVisitingCardPreview() {
      ...
      const profile = await loadBusinessProfile(auth.currentUser);
      const savedVisitingCard = await loadVisitingCardById(
        auth.currentUser,
        profile,
        visitingCardId,
      );
    ...
    async function hydrateReceiptPreview() {
      ...
      const profile = await loadBusinessProfile(auth.currentUser);
      const savedReceipt = await loadReceiptById(
        auth.currentUser,
        profile,
        receiptId,
      );
  ...
  async function handleFinalSave() {
    ...
    try {
      setSaving(true);
      const profile = await loadBusinessProfile(auth.currentUser);
      const result = await saveInvoice(auth.currentUser, profile, {
        ...invoice,
      ...
      Alert.alert(
        "Save Failed",
        error?.message ||
          "We could not save this document status. Please try again.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleQuotationFinalSave() {
    ...
    try {
      setSaving(true);
      const profile = await loadBusinessProfile(auth.currentUser);
      const result = await saveQuotation(auth.currentUser, profile, {
        ...quotation,
      ...
      Alert.alert(
        "Save Failed",
        error?.message ||
          "We could not save this quotation status. Please try again.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleLetterheadFinalSave() {
    ...
    try {
      setSaving(true);
      const profile = await loadBusinessProfile(auth.currentUser);
      const result = await saveLetterhead(auth.currentUser, profile, {
        ...letterhead,
        status: selectedLetterheadStatus,
      });
      ...
      Alert.alert(
        "Save Failed",
        error?.message ||
          "We could not save this letterhead status. Please try again.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleVisitingCardFinalSave() {
    ...
    try {
      setSaving(true);
      const profile = await loadBusinessProfile(auth.currentUser);
      const result = await saveVisitingCard(auth.currentUser, profile, {
        ...visitingCard,
        status: selectedVisitingCardStatus,
      });
      ...
      Alert.alert(
        "Save Failed",
        error?.message ||
          "We could not save this visiting card status. Please try again.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleReceiptFinalSave() {
    ...
    try {
      setSaving(true);
      const profile = await loadBusinessProfile(auth.currentUser);
      const result = await saveReceipt(auth.currentUser, profile, {
        ...receipt,
      ...
      Alert.alert(
        "Save Failed",
        error?.message ||
          "We could not save this receipt status. Please try again.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleVisitingCardDuplicate() {
    ...
    try {
      setSaving(true);
      const profile = await loadBusinessProfile(auth.currentUser);
      const duplicate = duplicateVisitingCardRecord(visitingCard, [
        visitingCard,
      ]);
      const result = await saveVisitingCard(
        auth.currentUser,
        profile,
        duplicate,
      );
      Alert.alert(
        "Visiting Card Duplicated",
        `${result.card.fullName} was duplicated as a draft.`,
        [
          {
            text: "Edit",
            onPress: () =>
              router.replace(
                appRoute("/visiting-card", {
                  editCardId: result.card.id || "",
                }) as never,
              ),
          },
          { text: "Stay" },
      ...
      Alert.alert(
        "Duplicate Failed",
        error?.message || "We could not duplicate this visiting card.",
      );
    } finally {
      setSaving(false);
    }
  ...
  function handlePrint() {
    ...
    Alert.alert(
      "Print",
      "Native print/PDF export needs the Expo print package; web print is available in this build.",
    );
  }
        ...
        <Animated.View entering={FadeIn.duration(300)} style={{ flex: 1 }}>
          <View style={[styles.header, isWebsite && styles.webHeader]}>
            <Pressable
              ...
              onPress={() => router.replace(appRoute("/invoice") as never)}
            >
              <Ionicons name="chevron-back" size={22} color={theme.ink} />
            </Pressable>
            <Text style={styles.headerTitle}>
              {invoice
                ? `${getDocumentLabel(invoice.documentType)} Preview`
                : "Document Preview"}
            </Text>
              ...
              {loading ? (
                ...
                <View style={styles.emptyCard}>
                  ...
                  <Text style={styles.emptyText}>
                    We could not load this saved document.
                  </Text>
                </View>
              )}
            </ScrollView>
      ...
      <SafeAreaView style={[styles.safeArea, isWebsite && styles.webSafeArea]}>
        <Animated.View entering={FadeIn.duration(300)} style={{ flex: 1 }}>
          <View style={[styles.header, isWebsite && styles.webHeader]}>
            <Pressable
              style={styles.backButton}
              onPress={() => router.replace(appRoute("/quotation") as never)}
            >
              <Ionicons name="chevron-back" size={22} color={theme.ink} />
            </Pressable>
            <Text style={styles.headerTitle}>
              {quotation
                ? `${getQuotationLabel(quotation.documentType)} Preview`
                : "Quotation Preview"}
            </Text>
            <View style={styles.headerSpacer} />
          </View>
            ...
            >
              {loading ? (
                ...
                <View style={styles.emptyCard}>
                  ...
                  <Text style={styles.emptyText}>
                    We could not load this saved quotation.
                  </Text>
                </View>
              )}
            </ScrollView>
      ...
      <SafeAreaView style={[styles.safeArea, isWebsite && styles.webSafeArea]}>
        <Animated.View entering={FadeIn.duration(300)} style={{ flex: 1 }}>
          <View style={[styles.header, isWebsite && styles.webHeader]}>
            <Pressable
              style={styles.backButton}
              onPress={() => router.replace(appRoute("/letterhead") as never)}
            >
              <Ionicons name="chevron-back" size={22} color={theme.ink} />
            </Pressable>
            <Text style={styles.headerTitle}>
              {letterhead ? "Letterhead Preview" : "Letterhead"}
            </Text>
            <View style={styles.headerSpacer} />
          </View>
            ...
            >
              {loading ? (
                  ...
                  <View style={styles.workflowBar}>
                    ...
                    <View style={styles.statusGrid}>
                      {letterheadStatusOptions.map((option) => (
                        <Pressable
                          ...
                          style={[
                            styles.statusBox,
                            selectedLetterheadStatus === option.value &&
                              styles.statusBoxActive,
                          ]}
                          onPress={() =>
                            setSelectedLetterheadStatus(option.value)
                          }
                        >
                          <Text
                            style={[
                    ...
                    <View style={styles.previewActions}>
                      ...
                      selectedLetterheadStatus === "draft" ? (
                        <Pressable
                          ...
                          onPress={() =>
                            router.replace(
                              appRoute("/letterhead", {
                                editLetterheadId: letterhead.id || "",
                              }) as never,
                            )
                          }
                        >
                      ...
                      <Pressable
                        ...
                        onPress={handleLetterheadFinalSave}
                        disabled={saving}
                      >
                        <Text style={styles.primaryActionText}>
                          {saving ? "Saving" : "Final Save"}
                ...
                <View style={styles.emptyCard}>
                  ...
                  <Text style={styles.emptyText}>
                    We could not load this saved letterhead.
                  </Text>
                </View>
              )}
            </ScrollView>
        ...
        <Animated.View entering={FadeIn.duration(300)} style={{ flex: 1 }}>
          <View style={[styles.header, isWebsite && styles.webHeader]}>
            <Pressable
              style={styles.backButton}
              onPress={() =>
                router.replace(appRoute("/visiting-card") as never)
              }
            >
              <Ionicons name="chevron-back" size={22} color={theme.ink} />
            </Pressable>
            <Text style={styles.headerTitle}>
              {visitingCard ? "Visiting Card Preview" : "Visiting Card"}
            </Text>
            <View style={styles.headerSpacer} />
          </View>
            ...
            >
              {loading ? (
                ...
                <>
                  <View style={styles.workflowBar}>
                    ...
                    <View style={styles.statusGrid}>
                      {visitingCardStatusOptions.map((option) => (
                        <Pressable
                          ...
                          style={[
                            styles.statusBox,
                            selectedVisitingCardStatus === option.value &&
                              styles.statusBoxActive,
                          ]}
                          onPress={() =>
                            setSelectedVisitingCardStatus(option.value)
                          }
                        >
                          <Text
                            style={[
                    ...
                    <View style={styles.previewActions}>
                      ...
                      <Pressable
                        style={styles.secondaryAction}
                        onPress={handleVisitingCardDuplicate}
                      >
                        <Ionicons
                          name="copy-outline"
                          size={16}
                      ...
                      <Pressable
                        ...
                        onPress={handleVisitingCardFinalSave}
                        disabled={saving}
                      >
                        <Text style={styles.primaryActionText}>
                  ...
                  <View
                    style={[
                      styles.visitingCardPreviewGrid,
                      isDesktop && styles.visitingCardPreviewGridDesktop,
                    ]}
                  >
                ...
                <View style={styles.emptyCard}>
                  ...
                  <Text style={styles.emptyText}>
                    We could not load this saved visiting card.
                  </Text>
                </View>
              )}
            </ScrollView>
      ...
      <SafeAreaView style={[styles.safeArea, isWebsite && styles.webSafeArea]}>
        <Animated.View entering={FadeIn.duration(300)} style={{ flex: 1 }}>
          <View style={[styles.header, isWebsite && styles.webHeader]}>
            <Pressable
              style={styles.backButton}
              onPress={() => router.replace(appRoute("/receipt") as never)}
            >
              <Ionicons name="chevron-back" size={22} color={theme.ink} />
            </Pressable>
            <Text style={styles.headerTitle}>
              {receipt ? "Receipt Preview" : "Receipt"}
            </Text>
            <View style={styles.headerSpacer} />
          </View>
            ...
            >
              {loading ? (
                ...
                <View style={styles.emptyCard}>
                  ...
                  <Text style={styles.emptyText}>
                    We could not load this saved receipt.
                  </Text>
                </View>
              )}
            </ScrollView>
...
function ReceiptPreview({
  ...
  const isPaidReceipt =
    (receipt.receiptTitle || "").toUpperCase().includes("PAID") ||
    (receipt as any).receiptType === "paid";
  const currencySymbol =
    receipt.company.currency === "INR" || !receipt.company.currency
      ? "₹"
      : receipt.company.currency;
  ...
  if (isPaidReceipt) {
    ...
    return (
      <View
        style={[
          previewDocStyles.letterheadPaper,
          isDesktop && previewDocStyles.webLetterheadPaper,
          { padding: 32, minHeight: 1100, backgroundColor: "#FFFFFF" },
        ]}
      >
        {/* Top Header */}
        <View
        ...
        >
          <View style={{ flexDirection: "row", gap: 12, flex: 1 }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              >
                {receipt.company.name || "ABC ENTERPRISES PVT. LTD."}
              </Text>
              <Text
                style={{
                  fontSize: 11,
              ...
              >
                Documents that build your business
              </Text>
              ...
              <Text style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>
                📞 {receipt.company.phone || "+91 98765 43210"}
              </Text>
              <Text style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>
                ✉️ {receipt.company.email || "info@abcenterprises.com"}
              </Text>
              <Text style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>
                🌐 {receipt.company.website || "www.abcenterprises.com"}
              </Text>
            </View>
          </View>
          <View
          ...
          <View style={{ width: 220, gap: 4 }}>
            ...
            >
              ...
              <Text style={{ color: "#0F172A" }}>
                {receipt.company.taxRegistrationNumber || "27ABCDE1234F1Z5"}
              </Text>
            </Text>
            <Text
              style={{
            ...
            >
              PAN :{" "}
              <Text style={{ color: "#0F172A" }}>
                {(receipt.company as any).pan || "ABCDE1234F"}
              </Text>
            </Text>
            <Text
              style={{
            ...
            >
              CIN :{" "}
              <Text style={{ color: "#0F172A" }}>
                {(receipt.company as any).cin || "U74999MH2020PTC123456"}
              </Text>
            </Text>
        ...
        <View style={{ flexDirection: "row", marginTop: 20, gap: 20 }}>
          ...
          <View style={{ flex: 1, gap: 8 }}>
            ...
            <View style={{ flexDirection: "row" }}>
              ...
              <Text
                style={{ fontSize: 12, fontWeight: "700", color: "#0F172A" }}
              >
                : {receipt.receivedFrom.name || "Ramesh Kumar"}
              </Text>
            </View>
            <View style={{ flexDirection: "row" }}>
              <Text
              ...
              <Text
                style={{ fontSize: 12, fontWeight: "700", color: "#0F172A" }}
              >
                : {receipt.receivedFrom.phone || "+91 98765 67890"}
              </Text>
            </View>
            <View style={{ flexDirection: "row" }}>
              <Text
              ...
              <Text style={{ flex: 1, fontSize: 12, color: "#334155" }}>
                :{" "}
                {receipt.receivedFrom.address ||
                  "Civil Work, Building No. 5, Site Area..."}
              </Text>
            </View>
            <View style={{ flexDirection: "row" }}>
              <Text
            ...
            <View style={{ flexDirection: "row" }}>
              ...
              <Text style={{ flex: 1, fontSize: 12, color: "#334155" }}>
                : {receipt.notes || "Labour Payment (Construction Work)"}
              </Text>
            </View>
            <View style={{ flexDirection: "row" }}>
              <Text
              ...
              <Text style={{ flex: 1, fontSize: 12, color: "#334155" }}>
                : {receipt.paymentReference || "Daily Labour Payment"}
              </Text>
            </View>
          </View>
          ...
          <View style={{ width: 280, gap: 12 }}>
            ...
            >
              ...
              >
                ({receipt.amountInWords || "Rupees Five Thousand Only"})
              </Text>
            </View>
        ...
        >
          ...
          <Text style={{ fontSize: 12, color: "#334155", lineHeight: 18 }}>
            I, the undersigned, hereby confirm that I have received the sum of{" "}
            {currencySymbol} {receipt.amount.toFixed(2)} (
            {receipt.amountInWords || "Rupees Five Thousand Only"}) from{" "}
            {receipt.company.name} towards {receipt.notes || "Labour Payment"}{" "}
        ...
        >
          ...
          <View style={{ width: 220 }}>
            <Text style={{ fontSize: 12, fontWeight: "700", color: "#0F172A" }}>
              For {receipt.company.name}
            </Text>
            {receipt.company.signatureUrl ? (
              <Image
                source={{ uri: receipt.company.signatureUrl }}
                style={{ width: 120, height: 44, marginVertical: 4 }}
                contentFit="contain"
              />
            ) : (
              <View
          ...
          <View style={{ width: 200, alignItems: "flex-end" }}>
            ...
            <Text style={{ fontSize: 11, color: "#64748B" }}>
              Receiver Name : {receipt.receivedFrom.name || "Ramesh Kumar"}
            </Text>
            <Text style={{ fontSize: 11, color: "#64748B" }}>
        ...
        >
          ...
          <Text style={{ fontSize: 11, color: "#7C2D12" }}>
            • This is a payment made receipt.
          </Text>
          <Text style={{ fontSize: 11, color: "#7C2D12" }}>
            • This does not require any stamp or signature.
          </Text>
          <Text style={{ fontSize: 11, color: "#7C2D12" }}>
        ...
        >
          <Text style={{ fontSize: 11, color: "#475569" }}>
            📞 {receipt.company.phone} | ✉️ {receipt.company.email} | 🌐{" "}
    ...
    >
      ...
      <View style={{ flexDirection: "row", marginTop: 20, gap: 20 }}>
        ...
        <View style={{ flex: 1, gap: 8 }}>
          ...
          <View style={{ flexDirection: "row" }}>
            ...
            <Text style={{ fontSize: 12, fontWeight: "700", color: "#0F172A" }}>
              : {receipt.receivedFrom.name || "Rahul Traders"}
            </Text>
          </View>
          <View style={{ flexDirection: "row" }}>
            <Text
          ...
          <View style={{ flexDirection: "row" }}>
            ...
            <Text style={{ flex: 1, fontSize: 12, color: "#334155" }}>
              :{" "}
              {receipt.paymentReference || "Against Invoice No. INV-2024-0158"}
            </Text>
          </View>
          <View style={{ flexDirection: "row" }}>
            <Text
            ...
            <Text style={{ flex: 1, fontSize: 12, color: "#334155" }}>
              : {receipt.notes || "Full & Final Payment"}
            </Text>
          </View>
        </View>
            ...
            <View style={{ padding: 10, gap: 6 }}>
              ...
              >
                ...
                <Text
                  style={{ fontSize: 12, fontWeight: "700", color: "#0F172A" }}
                >
                  {currencySymbol} {(receipt.amount * 0.85).toFixed(2)}
                </Text>
              </View>
              <View
                style={{
              ...
              >
                ...
                <Text
                  style={{ fontSize: 12, fontWeight: "700", color: "#0F172A" }}
                >
                  {currencySymbol} {(receipt.amount * 0.075).toFixed(2)}
                </Text>
              </View>
              <View
                style={{
      ...
      >
        ...
        <Text style={{ fontSize: 12, color: "#334155" }}>
          ...
          {receipt.paymentReference || "INV-2024-0158"} towards supply of goods.
        </Text>
      </View>
  ...
  return (
    <DocumentPaperShell isDesktop={isDesktop}>
      <DocumentBrandHeader
        ...
        metaRows={[
          {
            label: isTaxInvoice ? "Invoice No." : "Bill No.",
      ...
      <DocumentTaxBar
        gstin={invoice.company.taxRegistrationNumber}
        ...
        {invoice.items.map((item, index) => (
          <View
            ...
            style={{
              ...
              backgroundColor:
                index % 2 ? DocumentColors.rowAlt : DocumentColors.paper,
            }}
          >
            <Text
              style={{
      ...
      >
        ...
        >
          <View style={{ padding: 8, gap: 4 }}>
            <SummaryRow
              label="Sub Total"
              value={formatMoney(totals.subtotal, currencySymbol)}
            />
            <SummaryRow
              label="Discount"
              value={formatMoney(invoice.discount, currencySymbol)}
            />
            <SummaryRow
              label="Taxable Value"
              value={formatMoney(totals.taxableValue, currencySymbol)}
            />
            {isTaxInvoice ? (
              <>
                <SummaryRow
                  label={`CGST ${invoice.cgstPercent}%`}
                  value={formatMoney(totals.cgstAmount, currencySymbol)}
                />
                <SummaryRow
                  label={`SGST ${invoice.sgstPercent}%`}
                  value={formatMoney(totals.sgstAmount, currencySymbol)}
                />
                <SummaryRow
                  label={`IGST ${invoice.igstPercent}%`}
                  value={formatMoney(totals.igstAmount, currencySymbol)}
                />
              </>
            ) : null}
          </View>
          <DocumentGrandTotalRow
            label="GRAND TOTAL"
            value={formatMoney(totals.grandTotal, currencySymbol)}
          />
        </View>
      </View>
      ...
      <View style={{ flexDirection: "row", gap: 16, marginTop: 20 }}>
        ...
        >
          <DocumentSectionTitle
            ...
            title="BANK DETAILS"
          />
          <Text style={previewDocStyles.muted}>
      ...
      <View style={previewDocStyles.signatureRow}>
        ...
        <View style={previewDocStyles.signBox}>
          <Text style={previewDocStyles.signFor}>
  ...
  return (
    ...
    >
      ...
      <View
        style={{
          position: "absolute",
          bottom: isTemplate1 ? 40 : "35%",
          right: isTemplate1 ? 40 : "25%",
          opacity: 0.06,
          pointerEvents: "none",
        }}
      >
      ...
      >
        <View style={{ flexDirection: "row", gap: 12, flex: 1 }}>
          ...
          <View>
            <Text style={{ fontSize: 20, fontWeight: "900", color: "#0F172A" }}>
              {letterhead.company.name || "ABC ENTERPRISES PVT. LTD."}
            </Text>
        ...
        <View style={{ alignItems: "flex-end", gap: 2 }}>
          ...
          <Text style={{ fontSize: 11, color: "#475569" }}>
            📞 {letterhead.company.phone || "+91 98765 43210"}
          </Text>
          <Text style={{ fontSize: 11, color: "#475569" }}>
            ✉️ {letterhead.company.email || "info@abcenterprises.com"}
          </Text>
          <Text style={{ fontSize: 11, color: "#475569" }}>
            🌐 {letterhead.company.website || "www.abcenterprises.com"}
          </Text>
        </View>
      </View>
      ...
      {isTemplate1 ? (
        ...
        >
          ...
          >
            GSTIN :{" "}
            <Text style={{ color: "#0F172A" }}>
              {letterhead.company.taxNumber || "27ABCDE1234F1Z5"}
            </Text>
          </Text>
          <Text
            style={{
          ...
          >
            CIN :{" "}
            <Text style={{ color: "#0F172A" }}>U74999MH2020PTC123456</Text>
          </Text>
        </View>
      ) : (
        <View
      ...
      <View style={{ flex: 1, paddingVertical: 20 }}>
        <Text style={{ fontSize: 15, color: "#0F172A", lineHeight: 26 }}>
          {letterhead.body ||
            "Type your official letterhead content here. Every section of this document is fully editable and backed up real-time to your workspace."}
        </Text>
      </View>
      ...
      >
        <Text style={{ fontSize: 11, color: "#475569" }}>
          📞 {letterhead.company.phone} | ✉️ {letterhead.company.email} | 🌐{" "}
          {letterhead.company.website}
        </Text>
        <Text
          style={{
            fontSize: 12,
...
function PreviewLine({
  ...
  return (
    <View
      style={[
        previewDocStyles.previewLine,
        compact && previewDocStyles.previewLineCompact,
      ]}
    >
...
function SummaryRow({
  ...
  return (
    <View style={previewDocStyles.summaryRow}>
      <Text
        style={[
          previewDocStyles.summaryLabel,
          strong && previewDocStyles.strong,
        ]}
      >
        {label}
      </Text>
  ...
  return (
    <View style={previewDocStyles.asset}>
      ...
      {uri ? (
        <Image
          source={{ uri }}
          style={previewDocStyles.assetImage}
          contentFit="contain"
        />
      ) : (
        <Text style={previewDocStyles.muted}>Not uploaded</Text>
      )}
    </View>
  );
}
...
const createStyles = (theme: ThemePalette, isDark: boolean) =>
  StyleSheet.create({
    ...
    letterheadPageNumber: {
      color: "#777777",
      fontSize: 9,
      marginTop: 4,
      textAlign: "right",
    },
    visitingCardPreviewGrid: {
      alignSelf: "center",
      gap: 18,
      maxWidth: 920,
      width: "100%",
    },
    visitingCardPreviewGridDesktop: {
      alignItems: "flex-start",
      flexDirection: "row",
    },
